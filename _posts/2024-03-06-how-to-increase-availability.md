...
다만 재시도를 할 때에는 주의해야 할 점들이 몇 가지 있습니다. RFC 9110에서는 프록시는 멱등하지 않은 요청은 절대 재시도하면 안 된다고 얘기합니다.[^3]
```text
...
```
어떤 요청이 멱등한지는 어플리케이션마다 다를 수 있으므로 로드밸런서에서는 이를 정확히 알 수 없습니다. 또한 로드밸런서와 클라이언트가 동시에 재시도를 한다면 요청이 증폭되어 오히려 장애 상황을 악화시킬 수도 있습니다. 

따라서 재시도를 통해 효과적으로 장애 회복을 하기 위해서는 먼저 일관성 있는 정책을 세워야 합니다. 예를 들어 이런 정책을 세울 수 있습니다.
